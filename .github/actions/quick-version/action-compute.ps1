        ...
        # Debug: dump INPUT_* environment vars to verify input names
        Write-Host "ENV INPUT_*:"
        Get-ChildItem env: | Where-Object { $_.Name -like 'INPUT_*' } | ForEach-Object { Write-Host (" - {0}={1}" -f $_.Name, $_.Value) }
        ...
        $stopMessages = $false
        $commitsSinceProd = 0
        $raw = git log --no-color --pretty=format:"%H%x01%B%x02"
        $records = ($raw -split ([char]2)) | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }
        foreach ($rec in $records) {
            $parts = $rec -split ([char]1), 2
            ...
            # guard against empty/invalid SHAs (prevents "ambiguous argument ''" from git)
            if ([string]::IsNullOrWhiteSpace($sha) -or -not ($sha -match '^[0-9a-f]{7,40}$')) { continue }

            # use '--' to separate revisions from paths and capture failures safely
            try {
                $filesRaw = git diff-tree --no-commit-id --name-only -r $sha -- 2>&1
                if ($LASTEXITCODE -ne 0) {
                    Write-Warning ("git diff-tree failed for commit '{0}': {1}" -f $sha, ($filesRaw -join "`n"))
                    $files = @()
                } else {
                    $files = $filesRaw | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }
                }
            } catch {
                Write-Warning ("git diff-tree exception for commit '{0}': {1}" -f $sha, $_)
                $files = @()
            }
            ...
            if (-not $stopMessages) {
                # Count this commit as "since last production" (only include commits that passed path filtering)
                $commitsSinceProd = $commitsSinceProd + 1
        ...
        Write-Host ""
        Write-Host "Inputs                       :"
        Write-Host " - version-label   : $($inputLabel)"
        ...
        Write-Host "Base Version                 : $($baseVersion.ToString())"
